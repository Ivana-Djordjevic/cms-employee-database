...
function renderEmployeeByManagers() {
    db.query('SELECT manager_id, GROUP_CONCAT(first_name, ' ', last_name) AS employee_names FROM employee GROUP BY manager_id;', (err, result) => {
        err ? console.log(err) : console.log(result)
    });
};

module.exports = { renderDepartmentData, renderRoleData, renderEmployeeData, addAdepartment, addArole, AddEmployee, determineRoleId, updateEmployee, determineEmployeedID, updateEmployeesManager, renderEmployeeByManagers };