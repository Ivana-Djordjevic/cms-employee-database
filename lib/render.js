...
//dynamically renders employee choices (includes employess added by user)
async function renderEmployeeInfo() {
    const result = await dbPromise.query('SELECT CONCAT (first_name, " ", last_name) AS name FROM employee;');
    return result[0].map(item => item.name)
}

//dynamically renders role choices (includes roles added by user)
async function renderRoleInfo() {
    const result = await dbPromise.query('SELECT title FROM role;');
    return result[0].map(item => item.title)
}

//dynamially renders departments choices (includes departments added by user)
async function renderDepartmentInfo() {
    const result = await dbPromise.query('SELECT name FROM department;');
    return result[0].map(item => item.name)
}

//interconnets manager_id with their name, that way user can select manager based upon their name (instead of manager_id)
async function renderManagerInfo() {
    const result = await dbPromise.query(`
        SELECT CONCAT (manager.first_name, " ", manager.last_name) AS name, employee.manager_id AS value 
        FROM employee 
        JOIN employee manager 
        ON employee.manager_id = manager.id;`);
    const filteredResult =  result[0].reduce((total,item) => {
        total[item.value] = item.name
        return total
    }, {})
    return Object.keys(filteredResult).map(key => ({value: key, name: filteredResult[key]}))
    // [...new Set(result[0])] <= a way to remove duplicates from an array
}
