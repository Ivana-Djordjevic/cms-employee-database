...
function renderBudgetOfEachDepartment() {
    db.query(`  SELECT department.name AS department_name, 
...
function deleteEmployee(employeeID) {
    db.query('DELETE FROM employee WHERE id = ?;', [employeeID], (err, result) => {
        err ? console.log(err) : console.log('successfully deleted employee')
    } );
};

module.exports = { renderDepartmentData, renderRoleData, renderEmployeeData, addAdepartment, addArole, AddEmployee, determineRoleId, updateEmployee, determineEmployeedID, updateEmployeesManager, renderEmployeesByManagers, renderEmployeesByDepartment, renderBudgetOfEachDepartment, deleteEmployee };