...
function determineRoleId(role, cb) {
    db.query('SELECT id FROM role WHERE title = ?', [role], (err, result) => {
        ...
        const roleId = result.length > 0 ? result[0].id : null;
        cb(roleId);
    });
};
...
function determineDepartmentId(department, cb) {
    db.query('SELECT id FROM department WHERE name = ?', [department], (err, result) => {
        if (err) console.log(err);
        const deparmentId = result.length > 0 ? result[0].id : null;
        cb(deparmentId);
    });
};

//function to delete the entirety of a department
function deleteEntireDepartment(deparmentId, init) {
    db.query('DELETE FROM department WHERE id = ?', [deparmentId], (err, result) => {
        err ? console.log(err) : cyanBlueText('successfully deleted a department');
    });
    db.query('DELETE FROM role WHERE department_id = ?', [deparmentId], (err, result) => {
        err ? console.log(err) : cyanBlueText('successfully deleted its role and employee(s)');
        init();
    });
};
